...
public class InventoryNavigationManager : MonoBehaviour
{
    ...
    public void DeactivateInventoryNavigation()
    {
        ...
        if (logNavigationActions)
            Debug.Log("InventoryNavigationManager: Manteniendo navegación activa para evitar conflictos");

        // Limpiar animaciones sin desactivar el componente
        CleanupAnimations();

        // CORREGIDO: NO deshabilitar inputs UI
        // playerControls.UI.Disable(); ← ELIMINADO

        // CORREGIDO: NO desactivar el sistema
        // isActive = false; ← ELIMINADO
        // this.enabled = false; ← ELIMINADO

        // Solo limpiar selección local
        ClearSelection();

        Debug.Log("Navegación del inventario mantenida activa (solo limpieza local realizada)");
    }
    ...
    {
        Debug.Log("Forzando reactivación de navegación del inventario...");
        ...
        Debug.Log("Navegación del inventario reactivada forzosamente");
    }
